...
class ExpApp {
    ...
    renderTasks() {
        ...
        this.ExpTasks.forEach((expTask: ExpTask) => {
            ...
            const taskText = expTask.completed ? `<span>${expTask.task}</span>` : expTask.task;

            const listItem = document.createElement("li");
            listItem.className = taskClass;
            listItem.innerHTML = `
                ${taskText}  ${expTask.amount}  ${expTask.category}  ${expTask.date}
                <button onclick="expApp.markCompleted(${expTask.id})" id="btn">Done</button>
                <button onclick="expApp.removeTask(${expTask.id})" id="btn">Delete</button>
            `;
...
export const expApp = new ExpApp();
